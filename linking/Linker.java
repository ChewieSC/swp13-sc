...
/**links the Data of DBPedia with the semantic chess poject
 * use Linker link = new Linker();link.makeNTOutputFile();
 * you have to upload your file manually
 * */
public class Linker 
...
//muss manuell dann geschehen
{
	 ...
	 * @param uri of the resource you wish to get a query
	 * @param attribut a property to the resource
	 * @param i to differ the resultvariable and remember the kind of attribut 
	 ...
	 * @param uri of the resource you wish to get a query
	 * @param attribut a property to the resource
	 * @param i to differ the resultvariable
	...
	{
		String query="{"+generateUnionPart(uri,"dbpedia-owl:birthDate",1)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:dateOfBirth",2)+"}UNION{"
				+generateUnionPart(uri,"dbpedia-owl:birthDate",3)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:dateOfDeath",4)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:deathDate",5)+"}UNION{"
				+generateUnionPart(uri,"dbpedia-owl:title",6)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:title",7)+"}UNION{"
				+generateUnionPart(uri,"dbpedia-owl:sportCountry",8)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:sportCountry",9)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:country",10)+"}UNION{"
				+generateUnionPart(uri,"dbpedia-owl:country",11)+"}UNION{"
				+generateUnionPart(uri,"dbpedia-owl:ranking",12)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:ranking",13)+"}UNION{"
				+generateUnionPart(uri,"dbpedia-owl:elo",14)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:elo",15)+"}UNION{"
				+generateUnionPart(uri,"dbpedia-owl:eloRecord",16)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:eloRecord",17)+"}UNION{"
				+generateUnionPart(uri,"dbpprop:peakrating",18)+"}";
				;
	...
	/**
	 * generates many Queries, sparqls dbpediaendpoint 
	 * and gives a nt-Syntax list of found attributes
	 * @param uri of the resource you wish to get information
	 * @param sourceUri the uri of the source which gets the new data
		...
		for(int i=0; i<queryString.length;i++)
		{
			...
			try 
			{
			    ...
			    while( results.hasNext()) 
			    {
			    	...
			    	int identifier= Integer.parseInt(resVar.get(0).toString().substring(4));
		    		String attribute="unknown", value=qs.get(resVar.get(0)).toString();
		    		String prop = 
		    		  "http://pcai042.informatik.uni-leipzig.de/~swp13-sc/ChessOntology/prop/";
		    		
		    		/*gives URIs <> and Non-URIs ""*/
		    		if((value.contains("http://"))){value="<"+value+"> ";}
		    		else{ value="\""+value+"\" ";}
		    		
		    		/*sets the attribute by the categorizationNumber*/
		    		if(identifier%10==0){attribute=prop+"birthDate";}
		    		if(identifier%10==1){attribute=prop+"deathDate";}
		    		if(identifier%10==2){attribute=prop+"titel";}
		    		if(identifier%10==3){attribute=prop+"nation";}
		    		if(identifier%10==4){attribute=prop+"elo";}
		    		if(identifier%10==5){attribute=prop+"birthPlace";}
		    		if(identifier%10==6){attribute=prop+"thumbnail";}
		    		if(identifier%10==7){attribute=prop+"peakRanking";}
		    		
		    		String comp="<"+ sourceUri + "> <" + attribute+"> "+value+".";
	...
	public String[] extractTupelFromMapping(int index, List list)
	{
		...
		{	
			if(temp.charAt(i)=='='){
				...
				srcflag=false;trgtflag=true;i+=2;
			}
			if(trgtflag==true){
	...
	public List<String> link()
	{
		...
		System.out.println("Getting Data, this takes its time (aprx. 20 minutes), \n" +
		...
		for (int i=0;i<mappingList.size();i++)
		{
			...
			if(i%10==0){System.out.println("Progress: "+ (double)i/mappingList.size());}
		}
	 ...
	 * @param filename name of the File you wish to upload
	 */
	...
	public static void main(String[] args)
	{
		...
		//System.out.println(link.generateQuery("http://dbpedia.org/resource/Jaime_Sunye_Neto", "dbpedia-owl:birthDate"));
		...
		//link.link();
		//link.getRessources("http://dbpedia.org/resource/Davor_Palo", "sourceUri");
